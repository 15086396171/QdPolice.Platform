...
function baiduDingWei() {
    ...
    geolocation.getCurrentPosition(function (r) {
        ...
        var point = new BMap.Point(r.point.lng, r.point.lat);  //获取当前地理名称
        var gc = new BMap.Geocoder();
        gc.getLocation(point, function (rs) {
            console.info(rs);


            $("#location").html(rs.address + rs.business); 

            $("#location").attr("Remark", r.point.lng + "," + r.point.lat);
        });
...
function gaodeDingWei() {
        ...
        mapObj.plugin('AMap.Geolocation', function () {
            geolocation = new AMap.Geolocation({
                ...
                //noGeoLocation: 3, //是否禁止使用IP定位，默认值为0，可选值0-3 ///0: 可以使用IP定位////1: 手机设备禁止使用IP定位///2: PC上禁止使用IP定位///3: 所有终端禁止使用IP定位
