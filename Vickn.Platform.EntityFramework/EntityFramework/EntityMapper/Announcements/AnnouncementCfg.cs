...
namespace Vickn.Platform.Announcements.EntityMapper
{
    ...
    /// 通知公告数据库映射配置
    /// </summary>
    public class AnnouncementCfg : EntityTypeConfiguration<Announcement>
    {
       	/// <summary>
        ///  构造方法[默认链接字符串< see cref = "PlatformDbContext" /> ]
        /// </summary>
		public AnnouncementCfg()
		{
		    ToTable("Announcement", PlatformConsts.SchemaName.Business);
		    ...
		    HasMany(p => p.AnnouncementUsers).WithRequired().HasForeignKey(p => p.AnnouncementId);
		}
    }
}